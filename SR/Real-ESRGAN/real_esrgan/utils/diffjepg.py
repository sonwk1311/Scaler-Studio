...
class DiffJPEG(nn.Module):
    ...
    def forward(self, x: Tensor, quality: Union[int, float, Tensor]) -> Tensor:
        factor = quality
...
class _YQuantize(nn.Module):
    ...
    def forward(self, x: Tensor, factor: Union[int, float, Tensor]) -> Tensor:
        if isinstance(factor, (int, float)):
...
class _CompressJPEG(nn.Module):
    ...
    def forward(self, x: Tensor, factor: Tensor) -> tuple[Tensor, Tensor, Tensor]:
...
class _DeCompressJPEG(nn.Module):
    ...
    def forward(self,
                ...
                image_height: int,
                image_width: int,
                factor: Union[int, float, Tensor]) -> Tensor:
        components = {"y": y, "cb": cb, "cr": cr}
        for k in components.keys():
            if k in ("cb", "cr"):
