...
def _estimate_aggd_parameters(vector: np.ndarray) -> tuple[np.ndarray, float, float]:
...
def _symm_pad_torch(im: torch.Tensor, padding: tuple[int, int, int, int]) -> torch.Tensor:
...
def _estimate_aggd_parameters_torch(tensor: torch.Tensor,
                                    get_sigma: bool) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
