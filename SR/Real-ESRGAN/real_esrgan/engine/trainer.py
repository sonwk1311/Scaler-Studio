...
def init_train_env(config_dict: DictConfig) -> tuple[DictConfig, torch.device]:
...
class Trainer:
    ...
    def degradation_transforms(self, gt: Tensor, gaussian_kernel1: Tensor, gaussian_kernel2: Tensor, sinc_kernel: Tensor) -> tuple[Tensor, Tensor, Tensor]:
    ...
    def train_psnr(self):
        ...
        for i, (gt, gaussian_kernel1, gaussian_kernel2, sic_kernel) in enumerate(self.train_dataloader):
            ...
            global_step = self.current_epoch * self.num_train_batch + i + 1

            # Record training log information
            if i % 100 == 0 or i == self.num_train_batch - 1:
                # Writer Loss to file
                self.tblogger.add_scalar("Train/Pixel_Loss", pixel_loss.item(), global_step)
    ...
    def train_gan(self):
        ...
        for i, (gt, gaussian_kernel1, gaussian_kernel2, sic_kernel) in enumerate(self.train_dataloader):
            ...
            if i % 100 == 0 or i == self.num_train_batch - 1:
                iters = i + self.current_epoch * self.num_train_batch + 1
